...
def test_service_initialization(mock_repo: MagicMock) -> None:
    """Tests that the service initializes correctly with a repository."""
    service = TaskService(repository=mock_repo)
    assert len(service.get_tasks()) == 1
    assert service.get_tasks()[0].text == "Initial Task"


def test_service_add_task(mock_repo: MagicMock) -> None:
    """Tests that a task can be added via the service."""
    service = TaskService(repository=mock_repo)
...
def test_service_toggle_task(mock_repo: MagicMock) -> None:
    """Tests that a task can be toggled via the service."""
    service = TaskService(repository=mock_repo)
...
def test_service_delete_task(mock_repo: MagicMock) -> None:
    """Tests that a task can be deleted via the service."""
    service = TaskService(repository=mock_repo)
